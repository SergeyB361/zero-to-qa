# Практика: parallel ui execution

PROMPTS = {'safe_parallel': 'Перечисли минимум три сценария, которые обычно safe для параллельного запуска.',
 'serial_only': 'Перечисли минимум два сценария, которые лучше оставлять serial-only.',
 'conflict_sources': 'Перечисли минимум три источника конфликтов при parallel run.'}

SUBMISSION = {'safe_parallel': [], 'serial_only': [], 'conflict_sources': []}

RULES = {'safe_parallel': {'type': 'list', 'min_items': 3, 'keywords': ['isolated', 'independent']},
 'serial_only': {'type': 'list', 'min_items': 2, 'keywords': ['shared', 'state']},
 'conflict_sources': {'type': 'list', 'min_items': 3, 'keywords': ['account', 'data']}}
