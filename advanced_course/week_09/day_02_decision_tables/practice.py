...
PROMPTS = {'conditions': 'Перечисли условия правила.',
 'actions': 'Перечисли возможные действия/результаты.',
 'rules': 'Задай минимум четыре правила decision table как список словарей {name, when, then}.'}

SUBMISSION = {'conditions': [], 'actions': [], 'rules': []}

RULES = {'conditions': {'type': 'list', 'min_items': 2, 'keywords': ['condition']},
 'actions': {'type': 'list', 'min_items': 2, 'keywords': ['action']},
 'rules': {'type': 'list_of_dicts',
           'min_items': 4,
           'required_keys': ['name', 'when', 'then'],
           'keywords': ['then']}}
