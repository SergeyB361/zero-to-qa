...
def main() -> None:
    conn = build_demo_db()
    print('=' * 60)
    print('Создать временный test_case')
    query = """INSERT INTO test_cases (id, title, area, priority) VALUES (10, 'Temp case', 'sandbox', 'low');"""
    print(query.strip())
    print(fetch_all(conn, query))

    print('=' * 60)
    print('Проверить, что временная запись появилась')
    query = """SELECT COUNT(*) FROM test_cases WHERE id = 10;"""
    print(query.strip())
    print(fetch_all(conn, query))

    print('=' * 60)
    print('Удалить временную запись')
    query = """DELETE FROM test_cases WHERE id = 10;"""
    print(query.strip())
    print(fetch_all(conn, query))
