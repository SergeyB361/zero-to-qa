...
def main() -> None:
    conn = build_demo_db()
    print('=' * 60)
    print('Test cases, у которых были и failed, и passed run')
    query = """SELECT c.title FROM test_cases AS c WHERE EXISTS (SELECT 1 FROM test_runs AS r WHERE r.case_id = c.id AND r.status = 'failed') AND EXISTS (SELECT 1 FROM test_runs AS r WHERE r.case_id = c.id AND r.status = 'passed') ORDER BY c.title;"""
    print(query.strip())
    print(fetch_all(conn, query))

    print('=' * 60)
    print('Открытые дефекты по severity')
    query = """SELECT severity, COUNT(*) AS defect_count FROM defects WHERE status = 'open' GROUP BY severity ORDER BY severity;"""
    print(query.strip())
    print(fetch_all(conn, query))

    print('=' * 60)
    print('Средняя длительность runs по area')
    query = """SELECT c.area, AVG(r.duration_sec) AS avg_duration FROM test_cases AS c JOIN test_runs AS r ON c.id = r.case_id GROUP BY c.area ORDER BY avg_duration DESC;"""
    print(query.strip())
    print(fetch_all(conn, query))
