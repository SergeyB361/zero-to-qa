...
def main() -> None:
    conn = build_demo_db()
    print('=' * 60)
    print('Получить список пользователей')
    query = """SELECT id, name FROM users ORDER BY id;"""
    print(query.strip())
    print(fetch_all(conn, query))

    print('=' * 60)
    print('Получить одно скалярное значение')
    query = """SELECT COUNT(*) AS run_count FROM test_runs;"""
    print(query.strip())
    print(fetch_all(conn, query))

    print('=' * 60)
    print('Прочитать только failed runs')
    query = """SELECT id, status FROM test_runs WHERE status = 'failed' ORDER BY id;"""
    print(query.strip())
    print(fetch_all(conn, query))
