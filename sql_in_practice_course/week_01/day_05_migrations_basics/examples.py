...
def main() -> None:
    conn = build_demo_db()
    conn.execute("ALTER TABLE qa_engineers ADD COLUMN squad TEXT")
    print(fetch_all(conn, "PRAGMA table_info('qa_engineers')"))
    conn.execute("CREATE TABLE schema_migrations (id INTEGER PRIMARY KEY, name TEXT NOT NULL, applied_at TEXT NOT NULL)")
    conn.execute("INSERT INTO schema_migrations VALUES (1, 'add_squad_to_engineers', '2026-04-12 10:00:00')")
    print(fetch_all(conn, "SELECT * FROM schema_migrations"))
