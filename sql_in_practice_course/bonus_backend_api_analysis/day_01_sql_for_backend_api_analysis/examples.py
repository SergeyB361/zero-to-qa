...
def main() -> None:
    ...
    print(fetch_all(conn, "SELECT rel.build_tag, COUNT(*) AS error_count FROM api_checks a JOIN releases rel ON rel.id = a.release_id WHERE a.status_code >= 400 GROUP BY rel.id, rel.build_tag ORDER BY rel.deployed_at"))
    print(fetch_all(conn, "SELECT rel.build_tag FROM defects d JOIN releases rel ON rel.id = d.release_id WHERE d.severity = 'critical'"))
