...
| # | Тема | Ключевые понятия |
|---|------|-----------------|
| 1 | Классы и объекты       | class, __init__, self, атрибуты класса и объекта |
| 2 | Методы                 | @classmethod, @staticmethod, __str__ |
| 3 | Наследование           | super(), isinstance(), issubclass(), полиморфизм |
| 4 | Инкапсуляция           | public/protected/private, @property, name mangling |
...
## Темп и прогноз по базовому курсу

> Зафиксировано по состоянию на 2026-03-24

- Старт базового курса: `2026-02-26`
- Текущий прогресс: `20 из 84` учебных дней основной программы
- Средний темп: около `0.74` учебного дня в календарный день
- Средний темп: около `5.2` учебного дня в неделю

### Прогноз завершения базового курса
- Базовый прогноз при текущем темпе: `середина июня 2026`
- Реалистичный диапазон: `2026-06-15` — `2026-06-25`
- Оптимистичный сценарий: `2026-06-01` — `2026-06-10`
- Консервативный сценарий: `2026-07-01` — `2026-07-15`

### Комментарий
- Темп оценивается как высокий и устойчивый.
- Основной фактор, который может замедлять прохождение, — переключение времени с учёбы на перестройку и улучшение самого курса.
...
- [ ] Доп. проект недели — Зоопарк (необязательно)
...
## БОНУСНЫЙ БЛОК — ADVANCED PYTHON OOP

> Не входит в основной прогресс расширенного курса. Используется как дополнительная Python-углублёнка после базового OOP и основных QA-тем.

### День 1 — `__str__` и `__repr__`
- читаемое представление объектов
- дебаг и логирование моделей

### День 2 — `__eq__` и `__hash__`
- сравнение объектов
- использование в `set` и `dict`

### День 3 — `__call__` и callable objects
- вызываемые объекты
- классы-декораторы

### День 4 — Дескрипторы и углублённый `property`
...
- контроль и прокси-логика

### День 6 — Data classes, multiple inheritance, `__slots__`
- `dataclasses` как инструмент моделей
- ограничения и риски сложного наследования
- оптимизация памяти через `__slots__`

### День 7 — Бонусный мини-проект: Rich domain model
- доменная модель с магическими методами
- читаемый `repr/str`
- сравнение, валидация и удобный API объекта

---

## ОТДЕЛЬНЫЙ КУРС — Алгоритмы и структуры данных

- Полный план вынесен в `algorithms_course/ROADMAP.md`.
- Курс не влияет на прогресс `base_course` и `advanced_course`.
- Предназначен как самостоятельный трек по алгоритмическому мышлению и структурам данных.
