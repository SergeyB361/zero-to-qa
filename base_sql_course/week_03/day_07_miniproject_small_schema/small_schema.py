...
def create_schema(conn: sqlite3.Connection):
    """Создай маленькую схему из users, projects и tasks."""
    return None


def seed_data(conn: sqlite3.Connection):
    """Добавь demo-данные в новую схему."""
    return None


def list_tables(conn: sqlite3.Connection):
    """Верни список таблиц новой схемы."""
    return []


def query_task_overview(conn: sqlite3.Connection):
    """Верни обзор задач с join по новой схеме."""
    return []


def main() -> None:
    conn = build_demo_db()
    print('create_schema ->', create_schema(conn), '| expected:', 'schema created')
    print('seed_data ->', seed_data(conn), '| expected:', 'seed inserted')
    print('list_tables ->', list_tables(conn), '| expected:', "['projects', 'tasks', 'users']")
    print('query_task_overview ->', query_task_overview(conn), '| expected:', '3 rows expected')
