...
def main() -> None:
    conn = build_demo_db()
    print('=' * 60)
    print('Test cases, у которых есть failed run')
    query = """SELECT c.title FROM test_cases AS c WHERE EXISTS (SELECT 1 FROM test_runs AS r WHERE r.case_id = c.id AND r.status = 'failed') ORDER BY c.title;"""
    print(query.strip())
    print(fetch_all(conn, query))

    print('=' * 60)
    print('Test cases без failed run')
    ...
    print('=' * 60)
    print('Пользователи, у которых есть open задачи')
    query = """SELECT u.name FROM users AS u WHERE EXISTS (SELECT 1 FROM tasks AS t WHERE t.assignee_id = u.id AND t.status = 'open') ORDER BY u.name;"""
    print(query.strip())
    print(fetch_all(conn, query))
