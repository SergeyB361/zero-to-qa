...
def cases_without_failed_runs(conn: sqlite3.Connection):
    """Верни test_cases, у которых нет failed run."""
    # Напиши SQL-запрос и выполни его через conn.execute(...).
    return []


def users_with_open_tasks(conn: sqlite3.Connection):
    """Верни имена пользователей, у которых есть open задачи."""
    # Напиши SQL-запрос и выполни его через conn.execute(...).
    return []


def projects_without_closed_tasks(conn: sqlite3.Connection):
    """Верни проекты, в которых нет задач со status = closed."""
    # Напиши SQL-запрос и выполни его через conn.execute(...).
    return []
...
def main() -> None:
    conn = build_demo_db()
    print('cases_without_failed_runs ->', cases_without_failed_runs(conn), '| expected:', "['Export report', 'Filter products']")
    print('users_with_open_tasks ->', users_with_open_tasks(conn), '| expected:', "['Anna', 'Nina']")
    print('projects_without_closed_tasks ->', projects_without_closed_tasks(conn), '| expected:', "['Mobile App', 'Public API']")
