...
def main() -> None:
    conn = build_demo_db()
    print('=' * 60)
    print('Создать таблицу releases')
    query = """CREATE TABLE releases (id INTEGER PRIMARY KEY, version TEXT NOT NULL);"""
    print(query.strip())
    print(fetch_all(conn, query))

    print('=' * 60)
    print('Добавить колонку released_at')
    query = """ALTER TABLE releases ADD COLUMN released_at TEXT;"""
    print(query.strip())
    print(fetch_all(conn, query))

    print('=' * 60)
    print('Удалить временную таблицу')
