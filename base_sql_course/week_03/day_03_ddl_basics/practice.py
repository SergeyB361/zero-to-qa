...
def create_releases_table(conn: sqlite3.Connection):
    """Создай таблицу releases и верни True, если она появилась."""
    # Напиши SQL-запрос и выполни его через conn.execute(...).
    return False


def add_released_at_column(conn: sqlite3.Connection):
    """Добавь колонку released_at и верни список колонок таблицы releases."""
    # Напиши SQL-запрос и выполни его через conn.execute(...).
    return []


def drop_temp_table(conn: sqlite3.Connection):
    """Создай и затем удали temp_results. Верни True, если таблицы больше нет."""
    # Напиши SQL-запрос и выполни его через conn.execute(...).
    return False
...
def main() -> None:
    conn = build_demo_db()
    print('create_releases_table ->', create_releases_table(conn), '| expected:', 'True')
    print('add_released_at_column ->', add_released_at_column(conn), '| expected:', "['id', 'version', 'released_at']")
    print('drop_temp_table ->', drop_temp_table(conn), '| expected:', 'True')
