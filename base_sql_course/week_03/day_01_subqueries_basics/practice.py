...
def tasks_above_average(conn: sqlite3.Connection):
    """Верни id задач, у которых estimate_hours выше среднего по tasks."""
    # Напиши SQL-запрос и выполни его через conn.execute(...).
    return []


def cases_with_failed_runs(conn: sqlite3.Connection):
    """Верни названия test_cases, у которых были failed runs."""
    # Напиши SQL-запрос и выполни его через conn.execute(...).
    return []


def owners_of_projects(conn: sqlite3.Connection):
    """Верни имена владельцев проектов."""
    # Напиши SQL-запрос и выполни его через conn.execute(...).
    return []
...
def main() -> None:
    conn = build_demo_db()
    print('tasks_above_average ->', tasks_above_average(conn), '| expected:', '[3, 5]')
    print('cases_with_failed_runs ->', cases_with_failed_runs(conn), '| expected:', "['Create order', 'Login works']")
    print('owners_of_projects ->', owners_of_projects(conn), '| expected:', "['Anna', 'Boris', 'Nina']")
