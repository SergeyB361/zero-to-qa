...
def main() -> None:
    conn = build_demo_db()
    print('=' * 60)
    print('Задачи длиннее среднего estimate')
    query = """SELECT id, estimate_hours FROM tasks WHERE estimate_hours > (SELECT AVG(estimate_hours) FROM tasks) ORDER BY id;"""
    print(query.strip())
    print(fetch_all(conn, query))

    print('=' * 60)
    print('Test cases с failed runs')
    query = """SELECT title FROM test_cases WHERE id IN (SELECT case_id FROM test_runs WHERE status = 'failed') ORDER BY title;"""
    print(query.strip())
    print(fetch_all(conn, query))

    print('=' * 60)
    print('Пользователи, которые владеют проектами')
    query = """SELECT name FROM users WHERE id IN (SELECT owner_id FROM projects) ORDER BY name;"""
    print(query.strip())
    print(fetch_all(conn, query))
