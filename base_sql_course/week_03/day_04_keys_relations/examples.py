...
def main() -> None:
    conn = build_demo_db()
    print('=' * 60)
    print('Посмотреть foreign keys таблицы tasks')
    query = """PRAGMA foreign_key_list('tasks');"""
    print(query.strip())
    print(fetch_all(conn, query))

    print('=' * 60)
    print('Посмотреть foreign keys таблицы test_runs')
    query = """PRAGMA foreign_key_list('test_runs');"""
    print(query.strip())
    print(fetch_all(conn, query))

    print('=' * 60)
    print('Связать project -> tasks -> users')
    query = """SELECT p.name, t.id, u.name FROM projects AS p JOIN tasks AS t ON p.id = t.project_id JOIN users AS u ON t.assignee_id = u.id ORDER BY p.id, t.id;"""
    print(query.strip())
    print(fetch_all(conn, query))
