...
def tasks_foreign_keys(conn: sqlite3.Connection):
    """Верни количество foreign keys у таблицы tasks."""
    # Напиши SQL-запрос и выполни его через conn.execute(...).
    return -1


def test_runs_foreign_keys(conn: sqlite3.Connection):
    """Верни количество foreign keys у таблицы test_runs."""
    # Напиши SQL-запрос и выполни его через conn.execute(...).
    return -1


def project_task_user_rows(conn: sqlite3.Connection):
    """Верни количество строк в join `projects -> tasks -> users`."""
    # Напиши SQL-запрос и выполни его через conn.execute(...).
    return -1
...
def main() -> None:
    conn = build_demo_db()
    print('tasks_foreign_keys ->', tasks_foreign_keys(conn), '| expected:', '2')
    print('test_runs_foreign_keys ->', test_runs_foreign_keys(conn), '| expected:', '2')
    print('project_task_user_rows ->', project_task_user_rows(conn), '| expected:', '5')
