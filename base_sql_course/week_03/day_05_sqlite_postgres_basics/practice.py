...
def sqlite_strengths(conn: sqlite3.Connection):
    """Верни список из трёх сильных сторон SQLite."""
    # Напиши SQL-запрос и выполни его через conn.execute(...).
    return []


def postgres_strengths(conn: sqlite3.Connection):
    """Верни список из трёх сильных сторон PostgreSQL."""
    # Напиши SQL-запрос и выполни его через conn.execute(...).
    return []


def choose_engine(conn: sqlite3.Connection):
    """Для строки case_name верни `sqlite` или `postgres`."""
    # Напиши SQL-запрос и выполни его через conn.execute(...).
    return ''
...
def main() -> None:
    conn = build_demo_db()
    print('sqlite_strengths ->', sqlite_strengths(conn), '| expected:', "['embedded', 'simple setup', 'good for local tests']")
    print('postgres_strengths ->', postgres_strengths(conn), '| expected:', "['server DB', 'better concurrency', 'richer SQL features']")
    print('choose_engine ->', choose_engine(conn), '| expected:', 'Например: local_demo -> sqlite, production_api -> postgres')
