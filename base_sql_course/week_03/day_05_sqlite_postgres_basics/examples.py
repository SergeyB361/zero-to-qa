...
def main() -> None:
    conn = build_demo_db()
    print('=' * 60)
    print('SQLite удобна для локальных тестов')
    query = """SELECT 'sqlite' AS engine, 'embedded and simple' AS note;"""
    print(query.strip())
    print(fetch_all(conn, query))

    print('=' * 60)
    print('SQLite version placeholder')
    query = """SELECT sqlite_version() AS version;"""
    print(query.strip())
    print(fetch_all(conn, query))

    print('=' * 60)
    print('PostgreSQL как production reference')
    query = """SELECT 'postgres' AS engine, 'server and concurrency' AS note;"""
    print(query.strip())
    print(fetch_all(conn, query))
