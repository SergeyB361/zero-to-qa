...
def main() -> None:
    conn = build_demo_db()
    print('=' * 60)
    print('Получить список колонок таблицы tasks')
    query = """PRAGMA table_info('tasks');"""
    print(query.strip())
    print(fetch_all(conn, query))

    print('=' * 60)
    print('Test cases без Login в названии')
    query = """SELECT title FROM test_cases WHERE title NOT LIKE '%Login%' ORDER BY title;"""
    print(query.strip())
    print(fetch_all(conn, query))

    print('=' * 60)
    print('Собрать задачи с проектом и исполнителем')
    query = """SELECT t.id, p.name AS project_name, u.name AS assignee FROM tasks AS t JOIN projects AS p ON p.id = t.project_id JOIN users AS u ON u.id = t.assignee_id ORDER BY t.id;"""
    print(query.strip())
    print(fetch_all(conn, query))
