...
def main() -> None:
    conn = build_demo_db()
    print('=' * 60)
    print('Проекты и количество задач')
    query = """SELECT p.name, COUNT(t.id) AS task_count FROM projects AS p LEFT JOIN tasks AS t ON p.id = t.project_id GROUP BY p.id, p.name ORDER BY p.id;"""
    print(query.strip())
    print(fetch_all(conn, query))

    print('=' * 60)
    print('Test cases и количество запусков')
    query = """SELECT c.title, COUNT(r.id) AS run_count FROM test_cases AS c LEFT JOIN test_runs AS r ON c.id = r.case_id GROUP BY c.id, c.title ORDER BY c.id;"""
    print(query.strip())
    print(fetch_all(conn, query))

    print('=' * 60)
    print('Пользователи и количество найденных дефектов')
    query = """SELECT u.name, COUNT(d.id) AS defect_count FROM users AS u LEFT JOIN defects AS d ON u.id = d.created_by GROUP BY u.id, u.name ORDER BY u.id;"""
    print(query.strip())
    print(fetch_all(conn, query))
