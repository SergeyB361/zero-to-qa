...
def avg_duration_per_case(conn: sqlite3.Connection):
    """Верни пары `case:avg_duration`."""
    # Напиши SQL-запрос и выполни его через conn.execute(...).
    return []


def task_count_per_user(conn: sqlite3.Connection):
    """Верни пары `user:count` по количеству задач."""
    # Напиши SQL-запрос и выполни его через conn.execute(...).
    return []


def projects_with_high_tasks(conn: sqlite3.Connection):
    """Верни пары `project:high_priority_count`."""
    # Напиши SQL-запрос и выполни его через conn.execute(...).
    return []
...
def main() -> None:
    conn = build_demo_db()
    print('avg_duration_per_case ->', avg_duration_per_case(conn), '| expected:', "['Create order:57.5', 'Export report:70.0', 'Filter products:0.0', 'Login works:38.0']")
    print('task_count_per_user ->', task_count_per_user(conn), '| expected:', "['Anna:2', 'Boris:1', 'Nina:1', 'Oleg:1']")
    print('projects_with_high_tasks ->', projects_with_high_tasks(conn), '| expected:', "['Web Portal:1', 'Public API:1', 'Mobile App:0']")
