...
def main() -> None:
    conn = build_demo_db()
    print('=' * 60)
    print('Средняя длительность runs по test case')
    query = """SELECT c.title, AVG(r.duration_sec) AS avg_duration FROM test_cases AS c INNER JOIN test_runs AS r ON c.id = r.case_id GROUP BY c.id, c.title ORDER BY avg_duration DESC;"""
    print(query.strip())
    print(fetch_all(conn, query))

    print('=' * 60)
    print('Количество задач по исполнителям')
    query = """SELECT u.name, COUNT(t.id) AS task_count FROM users AS u LEFT JOIN tasks AS t ON u.id = t.assignee_id GROUP BY u.id, u.name ORDER BY task_count DESC, u.name;"""
    print(query.strip())
    print(fetch_all(conn, query))

    print('=' * 60)
    print('Проекты с количеством high priority задач')
    query = """SELECT p.name, COUNT(t.id) AS high_count FROM projects AS p LEFT JOIN tasks AS t ON p.id = t.project_id AND t.priority = 'high' GROUP BY p.id, p.name ORDER BY p.id;"""
    print(query.strip())
    print(fetch_all(conn, query))
