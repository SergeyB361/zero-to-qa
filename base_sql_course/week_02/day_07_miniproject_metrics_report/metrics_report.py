...
def runs_per_status(conn: sqlite3.Connection):
    """Словарь status -> count."""
    return {}


def avg_duration_per_case(conn: sqlite3.Connection):
    """Средняя длительность по test case."""
    return {}


def open_defects_by_severity(conn: sqlite3.Connection):
    """Словарь severity -> count для open defects."""
    return {}


def main() -> None:
    conn = build_demo_db()
    print('runs_per_status ->', runs_per_status(conn), '| expected:', "{'failed': 2, 'passed': 3, 'skipped': 1}")
