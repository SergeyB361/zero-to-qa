...
def task_assignees(conn: sqlite3.Connection):
    """Верни пары `task_id:name` для всех задач."""
    # Напиши SQL-запрос и выполни его через conn.execute(...).
    return []


def project_owners(conn: sqlite3.Connection):
    """Верни пары `project_name:owner_name`."""
    # Напиши SQL-запрос и выполни его через conn.execute(...).
    return []


def failed_case_titles(conn: sqlite3.Connection):
...
def main() -> None:
    conn = build_demo_db()
    print('task_assignees ->', task_assignees(conn), '| expected:', "['1:Anna', '2:Oleg', '3:Boris', '4:Anna', '5:Nina']")
    print('project_owners ->', project_owners(conn), '| expected:', "['Mobile App:Nina', 'Public API:Boris', 'Web Portal:Anna']")
    print('failed_case_titles ->', failed_case_titles(conn), '| expected:', "['Create order', 'Login works']")
