...
def main() -> None:
    conn = build_demo_db()
    print('=' * 60)
    print('Задачи вместе с именем исполнителя')
    query = """SELECT t.id, u.name, t.status FROM tasks AS t INNER JOIN users AS u ON t.assignee_id = u.id ORDER BY t.id;"""
    print(query.strip())
    print(fetch_all(conn, query))

    print('=' * 60)
    print('Проекты вместе с владельцем')
    query = """SELECT p.name, u.name AS owner_name FROM projects AS p INNER JOIN users AS u ON p.owner_id = u.id ORDER BY p.id;"""
    print(query.strip())
    print(fetch_all(conn, query))

    print('=' * 60)
    print('Запуски тестов вместе с названием test case')
    query = """SELECT r.id, c.title, r.status FROM test_runs AS r INNER JOIN test_cases AS c ON r.case_id = c.id ORDER BY r.id;"""
    print(query.strip())
    print(fetch_all(conn, query))
