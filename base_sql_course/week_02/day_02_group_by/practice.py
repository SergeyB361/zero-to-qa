...
def runs_per_status(conn: sqlite3.Connection):
    """Верни словарь status -> count для test_runs."""
    # Напиши SQL-запрос и выполни его через conn.execute(...).
    return {}


def tasks_per_priority(conn: sqlite3.Connection):
    """Верни словарь priority -> count для tasks."""
    # Напиши SQL-запрос и выполни его через conn.execute(...).
    return {}


def cases_per_area(conn: sqlite3.Connection):
    """Верни словарь area -> count для test_cases."""
    # Напиши SQL-запрос и выполни его через conn.execute(...).
    return {}
...
def main() -> None:
    ...
    print('tasks_per_priority ->', tasks_per_priority(conn), '| expected:', "{'high': 2, 'low': 1, 'medium': 2}")
    print('cases_per_area ->', cases_per_area(conn), '| expected:', "{'admin': 1, 'auth': 1, 'catalog': 1, 'checkout': 1}")
