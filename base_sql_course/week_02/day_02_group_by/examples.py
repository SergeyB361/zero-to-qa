...
def main() -> None:
    conn = build_demo_db()
    print('=' * 60)
    print('Количество пользователей по командам')
    query = """SELECT team, COUNT(*) AS user_count FROM users GROUP BY team ORDER BY team;"""
    print(query.strip())
    print(fetch_all(conn, query))

    print('=' * 60)
    print('Количество test_runs по статусу')
    query = """SELECT status, COUNT(*) AS run_count FROM test_runs GROUP BY status ORDER BY status;"""
    print(query.strip())
    print(fetch_all(conn, query))

    print('=' * 60)
    print('Средняя оценка задач по статусу')
    query = """SELECT status, AVG(estimate_hours) AS avg_estimate FROM tasks GROUP BY status ORDER BY status;"""
    print(query.strip())
    print(fetch_all(conn, query))
