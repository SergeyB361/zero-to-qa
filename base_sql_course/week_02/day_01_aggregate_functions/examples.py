...
def main() -> None:
    conn = build_demo_db()
    print('=' * 60)
    print('Количество test_runs')
    query = """SELECT COUNT(*) AS run_count FROM test_runs;"""
    print(query.strip())
    print(fetch_all(conn, query))

    print('=' * 60)
    print('Средняя длительность тест-рана')
    query = """SELECT AVG(duration_sec) AS avg_duration FROM test_runs;"""
    print(query.strip())
    print(fetch_all(conn, query))

    print('=' * 60)
    print('Максимальная оценка задачи')
    query = """SELECT MAX(estimate_hours) AS max_estimate FROM tasks;"""
    print(query.strip())
    print(fetch_all(conn, query))
