...
def count_failed_runs(conn: sqlite3.Connection):
    """Верни количество failed test_runs."""
    # Напиши SQL-запрос и выполни его через conn.execute(...).
    return -1


def sum_open_task_estimates(conn: sqlite3.Connection):
    """Верни сумму estimate_hours по open задачам."""
    # Напиши SQL-запрос и выполни его через conn.execute(...).
    return -1


def min_and_max_run_duration(conn: sqlite3.Connection):
    """Верни кортеж из минимальной и максимальной длительности test_runs."""
    # Напиши SQL-запрос и выполни его через conn.execute(...).
    return (-1, -1)
...
def main() -> None:
    conn = build_demo_db()
    print('count_failed_runs ->', count_failed_runs(conn), '| expected:', '2')
    print('sum_open_task_estimates ->', sum_open_task_estimates(conn), '| expected:', '20')
    print('min_and_max_run_duration ->', min_and_max_run_duration(conn), '| expected:', '(0, 70)')
