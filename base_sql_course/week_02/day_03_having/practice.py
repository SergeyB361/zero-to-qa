...
def statuses_with_multiple_runs(conn: sqlite3.Connection):
    """Верни статусы test_runs, у которых count > 1."""
    # Напиши SQL-запрос и выполни его через conn.execute(...).
    return []


def users_with_multiple_tasks(conn: sqlite3.Connection):
    """Верни id пользователей, у которых больше одной задачи."""
    # Напиши SQL-запрос и выполни его через conn.execute(...).
    return []


def projects_with_large_backlog(conn: sqlite3.Connection):
    """Верни id проектов, у которых сумма estimate_hours >= 10."""
    # Напиши SQL-запрос и выполни его через conn.execute(...).
    return []
...
def main() -> None:
    conn = build_demo_db()
    print('statuses_with_multiple_runs ->', statuses_with_multiple_runs(conn), '| expected:', "['failed', 'passed']")
    print('users_with_multiple_tasks ->', users_with_multiple_tasks(conn), '| expected:', '[1]')
    print('projects_with_large_backlog ->', projects_with_large_backlog(conn), '| expected:', '[2, 3]')
