...
def main() -> None:
    conn = build_demo_db()
    print('=' * 60)
    print('Статусы test_runs с количеством больше 1')
    query = """SELECT status, COUNT(*) AS run_count FROM test_runs GROUP BY status HAVING COUNT(*) > 1 ORDER BY status;"""
    print(query.strip())
    print(fetch_all(conn, query))

    print('=' * 60)
    print('Пользователи с более чем одной задачей')
    query = """SELECT assignee_id, COUNT(*) AS task_count FROM tasks GROUP BY assignee_id HAVING COUNT(*) > 1;"""
    print(query.strip())
    print(fetch_all(conn, query))

    print('=' * 60)
    print('Проекты с суммарной оценкой больше 5 часов')
    query = """SELECT project_id, SUM(estimate_hours) AS total_estimate FROM tasks GROUP BY project_id HAVING SUM(estimate_hours) > 5;"""
    print(query.strip())
    print(fetch_all(conn, query))
