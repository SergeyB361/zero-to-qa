...
def main() -> None:
    conn = build_demo_db()
    print('=' * 60)
    print('Открытые задачи без даты закрытия')
    query = """SELECT id, status FROM tasks WHERE closed_at IS NULL ORDER BY id;"""
    print(query.strip())
    print(fetch_all(conn, query))

    print('=' * 60)
    print('Test cases, начинающиеся на Create')
    query = """SELECT title FROM test_cases WHERE title LIKE 'Create%';"""
    print(query.strip())
    print(fetch_all(conn, query))

    print('=' * 60)
    print('Запуски средней длины')
    query = """SELECT id, duration_sec FROM test_runs WHERE duration_sec BETWEEN 35 AND 60 ORDER BY id;"""
    print(query.strip())
    print(fetch_all(conn, query))
