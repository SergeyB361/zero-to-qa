...
def main() -> None:
    conn = build_demo_db()
    print('open_tasks_without_closed_at ->', open_tasks_without_closed_at(conn), '| expected:', '[1, 4, 5]')
    print('areas_in_list ->', areas_in_list(conn), '| expected:', "['Create order', 'Login works']")
    print('medium_size_runs ->', medium_size_runs(conn), '| expected:', '[1, 2, 3, 6]')
