...
def distinct_areas(conn: sqlite3.Connection):
    """Верни список уникальных областей test_cases."""
    # Напиши SQL-запрос и выполни его через conn.execute(...).
    return []


def uppercase_user_names(conn: sqlite3.Connection):
    """Верни имена пользователей в верхнем регистре."""
    # Напиши SQL-запрос и выполни его через conn.execute(...).
    return []


def count_open_defects(conn: sqlite3.Connection):
    """Верни количество открытых дефектов."""
    # Напиши SQL-запрос и выполни его через conn.execute(...).
    return -1
...
def main() -> None:
    conn = build_demo_db()
    print('distinct_areas ->', distinct_areas(conn), '| expected:', "['admin', 'auth', 'catalog', 'checkout']")
    print('uppercase_user_names ->', uppercase_user_names(conn), '| expected:', "['ANNA', 'BORIS', 'NINA', 'OLEG']")
    print('count_open_defects ->', count_open_defects(conn), '| expected:', '2')
