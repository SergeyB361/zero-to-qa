...
def main() -> None:
    conn = build_demo_db()
    print('=' * 60)
    print('Уникальные команды')
    query = """SELECT DISTINCT team FROM users ORDER BY team;"""
    print(query.strip())
    print(fetch_all(conn, query))

    print('=' * 60)
    print('Переименовать колонки в выводе')
    query = """SELECT name AS user_name, team AS squad FROM users ORDER BY id;"""
    print(query.strip())
    print(fetch_all(conn, query))

    print('=' * 60)
    print('Показать длину заголовка test case')
    query = """SELECT title, LENGTH(title) AS title_len FROM test_cases ORDER BY id;"""
    print(query.strip())
    print(fetch_all(conn, query))
