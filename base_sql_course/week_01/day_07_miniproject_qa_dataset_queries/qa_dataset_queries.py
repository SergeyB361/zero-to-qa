...
def list_active_users(conn: sqlite3.Connection):
    """Список имён активных пользователей."""
    return []


def high_priority_cases(conn: sqlite3.Connection):
    """Список high priority test case."""
    return []


def open_task_ids(conn: sqlite3.Connection):
    """Список id открытых задач."""
    return []


def failed_run_ids(conn: sqlite3.Connection):
    """Список id failed test runs."""
    return []


def main() -> None:
    conn = build_demo_db()
    print('list_active_users ->', list_active_users(conn), '| expected:', "['Anna', 'Boris', 'Oleg']")
    print('high_priority_cases ->', high_priority_cases(conn), '| expected:', "['Create order', 'Login works']")
    print('open_task_ids ->', open_task_ids(conn), '| expected:', '[1, 4, 5]')
    print('failed_run_ids ->', failed_run_ids(conn), '| expected:', '[2, 6]')
