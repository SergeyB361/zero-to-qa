...
def high_priority_case_titles(conn: sqlite3.Connection):
    """Верни названия test case с priority = high."""
    # Напиши SQL-запрос и выполни его через conn.execute(...).
    return []


def active_users_ordered(conn: sqlite3.Connection):
    """Верни имена активных пользователей, отсортированных по team, затем name."""
    # Напиши SQL-запрос и выполни его через conn.execute(...).
    return []


def heavy_tasks(conn: sqlite3.Connection):
    """Верни id задач с estimate_hours >= 5."""
    # Напиши SQL-запрос и выполни его через conn.execute(...).
    return []
...
def main() -> None:
    conn = build_demo_db()
    print('high_priority_case_titles ->', high_priority_case_titles(conn), '| expected:', "['Create order', 'Login works']")
    print('active_users_ordered ->', active_users_ordered(conn), '| expected:', "['Boris', 'Anna', 'Oleg']")
    print('heavy_tasks ->', heavy_tasks(conn), '| expected:', '[1, 3, 5]')
