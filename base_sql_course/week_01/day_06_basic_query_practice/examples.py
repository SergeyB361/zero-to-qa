...
def main() -> None:
    conn = build_demo_db()
    print('=' * 60)
    print('Активные пользователи не из mobile')
    query = """SELECT name FROM users WHERE is_active = 1 AND team != 'mobile' ORDER BY name;"""
    print(query.strip())
    print(fetch_all(conn, query))

    print('=' * 60)
    print('Высокоприоритетные test case')
    query = """SELECT title FROM test_cases WHERE priority = 'high' ORDER BY title;"""
    print(query.strip())
    print(fetch_all(conn, query))

    print('=' * 60)
    print('Открытые или in_progress задачи')
    query = """SELECT id, status FROM tasks WHERE status IN ('open', 'in_progress') ORDER BY id;"""
    print(query.strip())
    print(fetch_all(conn, query))
