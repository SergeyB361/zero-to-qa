...
def main() -> None:
    conn = build_demo_db()
    print('=' * 60)
    print('Посмотреть список таблиц')
    query = """SELECT name FROM sqlite_master WHERE type = 'table' ORDER BY name;"""
    print(query.strip())
    print(fetch_all(conn, query))

    print('=' * 60)
    print('Прочитать первые строки users')
    query = """SELECT id, name, team FROM users ORDER BY id LIMIT 3;"""
    print(query.strip())
    print(fetch_all(conn, query))

    print('=' * 60)
    print('Посчитать количество проектов')
