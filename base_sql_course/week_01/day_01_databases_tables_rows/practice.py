...
def list_table_names(conn: sqlite3.Connection):
    """Верни список имён таблиц, отсортированный по имени."""
    # Напиши SQL-запрос и выполни его через conn.execute(...).
    return []


def count_users(conn: sqlite3.Connection):
    """Верни количество пользователей в users."""
    # Напиши SQL-запрос и выполни его через conn.execute(...).
    return -1


def list_active_teams(conn: sqlite3.Connection):
    """Верни уникальные команды активных пользователей."""
    # Напиши SQL-запрос и выполни его через conn.execute(...).
    return []
...
def main() -> None:
    conn = build_demo_db()
    print('list_table_names ->', list_table_names(conn), '| expected:', "['defects', 'projects', 'tasks', 'test_cases', 'test_runs', 'users']")
    print('count_users ->', count_users(conn), '| expected:', '4')
    print('list_active_teams ->', list_active_teams(conn), '| expected:', "['api', 'web']")
