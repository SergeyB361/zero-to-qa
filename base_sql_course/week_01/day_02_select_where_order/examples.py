...
def main() -> None:
    conn = build_demo_db()
    print('=' * 60)
    print('Активные пользователи по имени')
    query = """SELECT name, team FROM users WHERE is_active = 1 ORDER BY name;"""
    print(query.strip())
    print(fetch_all(conn, query))

    print('=' * 60)
    print('Две самые длинные задачи')
    query = """SELECT id, status, estimate_hours FROM tasks ORDER BY estimate_hours DESC LIMIT 2;"""
    print(query.strip())
    print(fetch_all(conn, query))

    print('=' * 60)
    print('Только открытые high priority задачи')
    query = """SELECT id, assignee_id FROM tasks WHERE status = 'open' AND priority = 'high';"""
    print(query.strip())
    print(fetch_all(conn, query))
