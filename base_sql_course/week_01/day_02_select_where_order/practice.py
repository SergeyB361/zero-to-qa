...
def top_two_open_tasks(conn: sqlite3.Connection):
    """Верни id двух открытых задач с максимальной оценкой."""
    # Напиши SQL-запрос и выполни его через conn.execute(...).
    return []


def active_web_users(conn: sqlite3.Connection):
    """Верни имена активных пользователей из команды web."""
    # Напиши SQL-запрос и выполни его через conn.execute(...).
    return []


def projects_sorted(conn: sqlite3.Connection):
    """Верни имена проектов в алфавитном порядке."""
    # Напиши SQL-запрос и выполни его через conn.execute(...).
    return []
...
def main() -> None:
    conn = build_demo_db()
    print('top_two_open_tasks ->', top_two_open_tasks(conn), '| expected:', '[5, 1]')
    print('active_web_users ->', active_web_users(conn), '| expected:', "['Anna', 'Oleg']")
    print('projects_sorted ->', projects_sorted(conn), '| expected:', "['Mobile App', 'Public API', 'Web Portal']")
