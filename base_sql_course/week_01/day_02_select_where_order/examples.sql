...
-- Пример 1: отфильтровать и отсортировать активных пользователей
SELECT name, team
FROM users
WHERE is_active = 1
ORDER BY name ASC;

-- Пример 2: взять две самые большие open-задачи по оценке
SELECT id, status, estimate_hours
FROM tasks
WHERE status = 'open'
ORDER BY estimate_hours DESC
LIMIT 2;

-- Пример 3: выбрать только нужные строки по нескольким условиям
