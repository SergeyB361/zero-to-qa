...
def insert_case_and_count(conn: sqlite3.Connection):
    """Добавь новый test case и верни новое количество test_cases."""
    # Напиши SQL-запрос и выполни его через conn.execute(...).
    return -1


def close_task_and_get_status(conn: sqlite3.Connection):
    """Закрой задачу с id = 4 и верни её новый статус."""
    # Напиши SQL-запрос и выполни его через conn.execute(...).
    return ''


def delete_closed_defects_and_count(conn: sqlite3.Connection):
    """Удали закрытые дефекты и верни количество оставшихся дефектов."""
    # Напиши SQL-запрос и выполни его через conn.execute(...).
    return -1
...
def main() -> None:
    conn = build_demo_db()
    print('insert_case_and_count ->', insert_case_and_count(conn), '| expected:', '5')
    print('close_task_and_get_status ->', close_task_and_get_status(conn), '| expected:', 'closed')
    print('delete_closed_defects_and_count ->', delete_closed_defects_and_count(conn), '| expected:', '2')
