...
def main() -> None:
    conn = build_demo_db()
    print('=' * 60)
    print('Добавить нового пользователя')
    query = """INSERT INTO users (id, name, team, is_active) VALUES (5, 'Roma', 'api', 1);"""
    print(query.strip())
    print(fetch_all(conn, query))

    print('=' * 60)
    print('Перевести задачу в closed')
    query = """UPDATE tasks SET status = 'closed', closed_at = '2026-04-12' WHERE id = 1;"""
    print(query.strip())
    print(fetch_all(conn, query))

    print('=' * 60)
    print('Удалить закрытый дефект')
    query = """DELETE FROM defects WHERE id = 2;"""
    print(query.strip())
    print(fetch_all(conn, query))
