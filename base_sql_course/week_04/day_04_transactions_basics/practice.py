...
def commit_task_close(conn: sqlite3.Connection):
    """Закрой задачу id = 3 в транзакции и верни итоговый статус."""
    # Напиши SQL-запрос и выполни его через conn.execute(...).
    return ''


def rollback_defect_close(conn: sqlite3.Connection):
    """Попробуй закрыть defect id = 1 и откати. Верни итоговый статус."""
    # Напиши SQL-запрос и выполни его через conn.execute(...).
    return ''


def transactional_insert_and_rollback(conn: sqlite3.Connection):
    """Вставь временного пользователя в транзакции и откати. Верни count строк с этим id."""
    # Напиши SQL-запрос и выполни его через conn.execute(...).
    return -1
...
def main() -> None:
    conn = build_demo_db()
    print('commit_task_close ->', commit_task_close(conn), '| expected:', 'closed')
    print('rollback_defect_close ->', rollback_defect_close(conn), '| expected:', 'open')
    print('transactional_insert_and_rollback ->', transactional_insert_and_rollback(conn), '| expected:', '0')
