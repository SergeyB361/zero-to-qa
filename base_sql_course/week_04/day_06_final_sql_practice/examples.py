...
def main() -> None:
    conn = build_demo_db()
    print('=' * 60)
    print('Собрать summary по runs и defects')
    query = """SELECT (SELECT COUNT(*) FROM test_runs) AS runs_total, (SELECT COUNT(*) FROM defects WHERE status = 'open') AS open_defects;"""
    print(query.strip())
    print(fetch_all(conn, query))

    print('=' * 60)
    print('Показать задачи с именем исполнителя и проектом')
    query = """SELECT t.id, p.name AS project_name, u.name AS assignee, t.status FROM tasks AS t JOIN projects AS p ON p.id = t.project_id JOIN users AS u ON u.id = t.assignee_id ORDER BY t.id;"""
    print(query.strip())
    print(fetch_all(conn, query))

    print('=' * 60)
    print('Вывести test_cases без failed runs')
    query = """SELECT c.title FROM test_cases AS c WHERE NOT EXISTS (SELECT 1 FROM test_runs AS r WHERE r.case_id = c.id AND r.status = 'failed') ORDER BY c.title;"""
    print(query.strip())
    print(fetch_all(conn, query))
