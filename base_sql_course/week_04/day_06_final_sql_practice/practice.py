...
def summary_counts(conn: sqlite3.Connection):
    """Верни словарь с общим количеством test_runs и open defects."""
    # Напиши SQL-запрос и выполни его через conn.execute(...).
    return {}


def task_overview_rows(conn: sqlite3.Connection):
    """Верни количество строк в обзоре задач с проектом и исполнителем."""
    # Напиши SQL-запрос и выполни его через conn.execute(...).
    return -1


def stable_case_titles(conn: sqlite3.Connection):
    """Верни test_cases, у которых не было failed run."""
    # Напиши SQL-запрос и выполни его через conn.execute(...).
    return []
...
def main() -> None:
    conn = build_demo_db()
    print('summary_counts ->', summary_counts(conn), '| expected:', "{'runs_total': 6, 'open_defects': 2}")
    print('task_overview_rows ->', task_overview_rows(conn), '| expected:', '5')
    print('stable_case_titles ->', stable_case_titles(conn), '| expected:', "['Export report', 'Filter products']")
