...
-- Пример 1: сгруппировать события по дням
...
-- Пример 2: получить bucket по месяцу
SELECT id,
       strftime('%Y-%m', executed_at) AS month_bucket
FROM test_runs
ORDER BY id;

-- Пример 3: вычислить следующий день после первой даты
SELECT date(MIN(executed_at), '+1 day') AS next_day
FROM test_runs;