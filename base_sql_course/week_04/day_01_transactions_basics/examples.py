...
def main() -> None:
    conn = build_demo_db()
    print('=' * 60)
    print('Открыть транзакцию и обновить статус задачи')
    query = """BEGIN; UPDATE tasks SET status = 'closed' WHERE id = 3; COMMIT;"""
    print(query.strip())
    conn.executescript(query)
    print(fetch_all(conn, "SELECT id, status FROM tasks WHERE id = 3;"))

    print('=' * 60)
    print('Откатить изменение дефекта')
    query = """BEGIN; UPDATE defects SET status = 'closed' WHERE id = 1; ROLLBACK;"""
    print(query.strip())
    conn.executescript(query)
    print(fetch_all(conn, "SELECT id, status FROM defects WHERE id = 1;"))

    print('=' * 60)
    print('Проверить итоговый статус после rollback')
    query = """SELECT status FROM defects WHERE id = 1;"""
    print(query.strip())
