...
def plan_before_index(conn: sqlite3.Connection):
    """Верни текст плана для поиска по endpoint до индекса."""
    # Напиши SQL или Python + SQL по условию.
    return ''


def plan_after_index(conn: sqlite3.Connection):
    """Создай индекс и верни текст плана после него."""
    # Напиши SQL или Python + SQL по условию.
    return ''


def compare_plans(conn: sqlite3.Connection):
    """Верни True, если планы до и после отличаются."""
    # Напиши SQL или Python + SQL по условию.
    return False
...
def main() -> None:
    conn = build_demo_db()
    print('plan_before_index ->', plan_before_index(conn), '| expected:', 'План должен содержать SCAN')
    print('plan_after_index ->', plan_after_index(conn), '| expected:', 'План должен содержать INDEX')
    print('compare_plans ->', compare_plans(conn), '| expected:', 'True')
