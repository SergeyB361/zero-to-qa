...
def rewrite_failed_cases_with_exists(conn: sqlite3.Connection):
    """Верни названия test_cases с failed runs через EXISTS."""
    # Напиши SQL или Python + SQL по условию.
    return []


def rewrite_open_defects_cte(conn: sqlite3.Connection):
    """Верни пары `build_tag:open_defects` через CTE."""
    # Напиши SQL или Python + SQL по условию.
    return []


def same_result_check(conn: sqlite3.Connection):
    """Верни True, если два варианта запроса на failed cases дают одинаковый результат."""
    # Напиши SQL или Python + SQL по условию.
    return False
...
def main() -> None:
    conn = build_demo_db()
    print('rewrite_failed_cases_with_exists ->', rewrite_failed_cases_with_exists(conn), '| expected:', "['Create order', 'Login works', 'Refund order']")
    print('rewrite_open_defects_cte ->', rewrite_open_defects_cte(conn), '| expected:', "['build-102:1', 'build-103:1', 'build-104:1']")
    print('same_result_check ->', same_result_check(conn), '| expected:', 'True')
