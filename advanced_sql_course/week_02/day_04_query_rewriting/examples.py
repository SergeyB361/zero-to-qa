...
def main() -> None:
    conn = build_demo_db()
    in_query = "SELECT title FROM test_cases WHERE id IN (SELECT case_id FROM test_runs WHERE status = 'failed') ORDER BY title"
    exists_query = """
    SELECT c.title
    FROM test_cases c
    WHERE EXISTS (
        SELECT 1 FROM test_runs r
        WHERE r.case_id = c.id AND r.status = 'failed'
    )
    ORDER BY c.title
    """
    print(fetch_all(conn, in_query))
    print(fetch_all(conn, exists_query))
