...
def replace_select_star(conn: sqlite3.Connection):
    """Верни исправленный SQL без SELECT * для поиска failed runs."""
    # Напиши SQL или Python + SQL по условию.
    return ''


def spot_unnecessary_distinct(conn: sqlite3.Connection):
    """Верни True, если DISTINCT в запросе не нужен."""
    # Напиши SQL или Python + SQL по условию.
    return False


def rewrite_late_filter(conn: sqlite3.Connection):
    """Верни исправленный SQL с ранней фильтрацией."""
    # Напиши SQL или Python + SQL по условию.
    return ''
...
def main() -> None:
    conn = build_demo_db()
    print('replace_select_star ->', replace_select_star(conn), '| expected:', 'Ожидается строка SQL')
    print('spot_unnecessary_distinct ->', spot_unnecessary_distinct(conn), '| expected:', 'True')
    print('rewrite_late_filter ->', rewrite_late_filter(conn), '| expected:', 'Ожидается строка SQL')
