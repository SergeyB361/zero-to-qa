...
def main() -> None:
    bad_query = "SELECT * FROM test_runs WHERE status = 'failed'"
    better_query = "SELECT id, case_id, status, duration_sec FROM test_runs WHERE status = 'failed'"
    print('bad_query ->', bad_query)
    print('better_query ->', better_query)
    print('reason -> avoid unnecessary columns and transfer volume')
