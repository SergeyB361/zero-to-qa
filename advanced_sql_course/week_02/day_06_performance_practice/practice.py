...
def orders_plan_before_after(conn: sqlite3.Connection):
    """Верни True, если план запроса по `/orders` меняется после индекса."""
    # Напиши SQL или Python + SQL по условию.
    return False


def top_slow_endpoints(conn: sqlite3.Connection):
    """Верни endpoint, отсортированные по средней latency."""
    # Напиши SQL или Python + SQL по условию.
    return []


def failed_runs_index_usage(conn: sqlite3.Connection):
    """Верни текст плана для failed runs после индекса."""
    # Напиши SQL или Python + SQL по условию.
    return ''
...
def main() -> None:
    conn = build_demo_db()
    print('orders_plan_before_after ->', orders_plan_before_after(conn), '| expected:', 'True')
    print('top_slow_endpoints ->', top_slow_endpoints(conn), '| expected:', "['/payments/refund', '/reports', '/orders', '/login']")
    print('failed_runs_index_usage ->', failed_runs_index_usage(conn), '| expected:', 'План должен содержать INDEX')
