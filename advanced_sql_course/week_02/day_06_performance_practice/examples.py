...
def main() -> None:
    conn = build_demo_db()
    query = "SELECT endpoint, AVG(response_ms) AS avg_ms FROM api_checks GROUP BY endpoint ORDER BY avg_ms DESC"
    print(fetch_all(conn, query))
    print(explain_query_plan(conn, "SELECT * FROM api_checks WHERE endpoint = '/orders'"))
    conn.execute("CREATE INDEX idx_api_endpoint ON api_checks(endpoint)")
    print(explain_query_plan(conn, "SELECT * FROM api_checks WHERE endpoint = '/orders'"))
