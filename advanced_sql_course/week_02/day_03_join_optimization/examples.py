...
def main() -> None:
    conn = build_demo_db()
    query = """
    SELECT c.title, r.status
    FROM test_runs r
    JOIN test_cases c ON c.id = r.case_id
    WHERE r.status = 'failed'
    """
    print(explain_query_plan(conn, query))
    conn.execute("CREATE INDEX idx_runs_case_id ON test_runs(case_id)")
    conn.execute("CREATE INDEX idx_runs_status ON test_runs(status)")
    print(explain_query_plan(conn, query))
    print(fetch_all(conn, query + ' ORDER BY c.title'))
