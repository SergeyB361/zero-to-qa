...
def failed_runs_join_plan(conn: sqlite3.Connection):
    """Верни текст плана для join failed runs + test_cases."""
    # Напиши SQL или Python + SQL по условию.
    return ''


def filtered_join_result(conn: sqlite3.Connection):
    """Верни названия test_cases только с failed runs."""
    # Напиши SQL или Python + SQL по условию.
    return []


def add_join_indexes_and_plan(conn: sqlite3.Connection):
    """Создай индексы на join-колонках и верни обновлённый план."""
    # Напиши SQL или Python + SQL по условию.
    return ''
...
def main() -> None:
    conn = build_demo_db()
    print('failed_runs_join_plan ->', failed_runs_join_plan(conn), '| expected:', 'План должен быть читаемым')
    print('filtered_join_result ->', filtered_join_result(conn), '| expected:', "['Create order', 'Login works', 'Refund order']")
    print('add_join_indexes_and_plan ->', add_join_indexes_and_plan(conn), '| expected:', 'План должен содержать INDEX')
