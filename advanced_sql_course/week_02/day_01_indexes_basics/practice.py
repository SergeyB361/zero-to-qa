...
def create_runs_status_index(conn: sqlite3.Connection):
    """Создай индекс на `test_runs(status)` и верни True, если индекс появился."""
    # Напиши SQL или Python + SQL по условию.
    return False


def list_indexes_for_runs(conn: sqlite3.Connection):
    """Верни список индексов таблицы test_runs."""
    # Напиши SQL или Python + SQL по условию.
    return []


def query_plan_for_failed_runs(conn: sqlite3.Connection):
    """Верни план запроса по status = failed после создания индекса."""
    # Напиши SQL или Python + SQL по условию.
    return ''
...
def main() -> None:
    conn = build_demo_db()
    print('create_runs_status_index ->', create_runs_status_index(conn), '| expected:', 'True')
    print('list_indexes_for_runs ->', list_indexes_for_runs(conn), '| expected:', "['idx_test_runs_status']")
    print('query_plan_for_failed_runs ->', query_plan_for_failed_runs(conn), '| expected:', 'План должен содержать INDEX')
