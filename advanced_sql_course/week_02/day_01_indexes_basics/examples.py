...
def main() -> None:
    conn = build_demo_db()
    print(explain_query_plan(conn, "SELECT * FROM test_runs WHERE status = 'failed'"))
    conn.execute("CREATE INDEX idx_test_runs_status ON test_runs(status)")
    print(explain_query_plan(conn, "SELECT * FROM test_runs WHERE status = 'failed'"))
    print(fetch_all(conn, "PRAGMA index_list('test_runs')"))
