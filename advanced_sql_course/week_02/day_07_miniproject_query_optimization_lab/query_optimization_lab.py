...
def plan_for_failed_runs(conn: sqlite3.Connection):
    """Верни план для выборки failed runs."""
    return ''


def plan_for_orders_endpoint(conn: sqlite3.Connection):
    """Верни план для `/orders`."""
    return ''


def add_indexes(conn: sqlite3.Connection):
    """Создай нужные индексы."""
    return False
...
def main() -> None:
    conn = build_demo_db()
    print('plan_for_failed_runs ->', plan_for_failed_runs(conn), '| expected:', 'plan text')
    print('plan_for_orders_endpoint ->', plan_for_orders_endpoint(conn), '| expected:', 'plan text')
    print('add_indexes ->', add_indexes(conn), '| expected:', 'True')
