...
-- Пример 2
SELECT id, endpoint, status_code FROM api_checks WHERE status_code >= 500 ORDER BY id;

-- Пример 3
CREATE TABLE temp_quality (title TEXT);

-- Пример 4
INSERT INTO temp_quality(title) VALUES (?);

-- Пример 5
SELECT title, COUNT(*) AS cnt FROM temp_quality GROUP BY title HAVING COUNT(*) > 1;

-- Пример 6
SELECT COUNT(*) AS broken_owner_count FROM defects WHERE owner_id IS NULL;
