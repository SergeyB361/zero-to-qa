...
def main() -> None:
    conn = build_demo_db()
    conn.execute("CREATE VIEW open_defects_view AS SELECT title, severity, release_id FROM defects WHERE status = 'open'")
    print(fetch_all(conn, "SELECT * FROM open_defects_view ORDER BY title"))
    conn.execute("CREATE TABLE defect_snapshot AS SELECT release_id, COUNT(*) AS open_count FROM defects WHERE status = 'open' GROUP BY release_id")
    print(fetch_all(conn, "SELECT * FROM defect_snapshot ORDER BY release_id"))
