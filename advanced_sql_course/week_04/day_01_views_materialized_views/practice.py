...
def create_open_defects_view(conn: sqlite3.Connection):
    """Создай view open_defects_view и верни True, если она появилась."""
    # Напиши SQL или Python + SQL по условию.
    return False


def query_open_defects_view(conn: sqlite3.Connection):
    """Верни пары `title:severity` из open_defects_view."""
    # Напиши SQL или Python + SQL по условию.
    return []


def build_snapshot_table(conn: sqlite3.Connection):
    """Создай table snapshot по open defects и верни количество строк."""
    # Напиши SQL или Python + SQL по условию.
    return -1
...
def main() -> None:
    conn = build_demo_db()
    print('create_open_defects_view ->', create_open_defects_view(conn), '| expected:', 'True')
    print('query_open_defects_view ->', query_open_defects_view(conn), '| expected:', "['Login returns 401:major', 'Refund 502:critical', 'Slow export:minor']")
    print('build_snapshot_table ->', build_snapshot_table(conn), '| expected:', '3')
