...
-- Пример 2
CREATE VIEW open_defects_view AS SELECT title, severity, release_id FROM defects WHERE status = 'open';

-- Пример 3
SELECT * FROM open_defects_view ORDER BY title;

-- Пример 4
CREATE TABLE defect_snapshot AS SELECT release_id, COUNT(*) AS open_count FROM defects WHERE status = 'open' GROUP BY release_id;

-- Пример 5
SELECT * FROM defect_snapshot ORDER BY release_id;
