...
def failed_runs_by_release(conn: sqlite3.Connection):
    """Верни количество failed runs по релизам."""
    return []


def critical_defects(conn: sqlite3.Connection):
    """Верни critical defects."""
    return []


def data_quality_summary(conn: sqlite3.Connection):
    """Верни короткую сводку по качеству данных."""
    return {}
...
def main() -> None:
    ...
    print('critical_defects ->', critical_defects(conn), '| expected:', "['Refund 502']")
    print('data_quality_summary ->', data_quality_summary(conn), '| expected:', "{'5xx_checks': 2, 'open_defects': 3}")
