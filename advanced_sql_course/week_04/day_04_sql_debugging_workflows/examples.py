...
def main() -> None:
    conn = build_demo_db()
    print(fetch_all(conn, "SELECT COUNT(*) AS cnt FROM test_runs"))
    print(fetch_all(conn, "SELECT COUNT(*) AS cnt FROM test_runs r JOIN test_cases c ON c.id = r.case_id"))
    query = """
    WITH failed_runs AS (
        SELECT release_id
        FROM test_runs
    ...
    SELECT rel.build_tag, COUNT(*) AS failed_count
    FROM failed_runs f
    JOIN releases rel ON rel.id = f.release_id
    GROUP BY rel.id, rel.build_tag
