...
-- Пример 2
SELECT endpoint, AVG(response_ms) AS avg_ms FROM api_checks GROUP BY endpoint ORDER BY avg_ms DESC;

-- Пример 3
SELECT rel.build_tag, COUNT(*) AS failed_count FROM test_runs r JOIN releases rel ON rel.id = r.release_id WHERE r.status = 'failed' GROUP BY rel.id, rel.build_tag ORDER BY rel.deployed_at;
