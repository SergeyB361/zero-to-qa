...
def slowest_cases(conn: sqlite3.Connection):
    """Top-3 test case по средней длительности."""
    return []


def failed_runs_by_release(conn: sqlite3.Connection):
    """Количество failed runs по релизам."""
    return []


def api_latency_ranking(conn: sqlite3.Connection):
    """Ранжирование endpoint по средней latency."""
    return []


def defect_summary(conn: sqlite3.Connection):
    """Короткий отчёт по дефектам."""
    return {}


def main() -> None:
    conn = build_demo_db()
    print('slowest_cases ->', slowest_cases(conn), '| expected:', "['Export report', 'Refund order', 'Create order']")
    print('failed_runs_by_release ->', failed_runs_by_release(conn), '| expected:', "['build-101:1', 'build-102:2', 'build-104:1']")
    print('api_latency_ranking ->', api_latency_ranking(conn), '| expected:', "['/payments/refund:1', '/reports:2', '/orders:3', '/login:4']")
    print('defect_summary ->', defect_summary(conn), '| expected:', "{'open': 3, 'closed': 1}")
    print('Доведи функции до совпадения с expected и затем улучши вывод.')
