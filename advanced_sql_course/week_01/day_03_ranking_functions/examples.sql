...
-- Пример 2
WITH failed_by_case AS (
        SELECT c.title, COUNT(*) AS failed_count
        FROM test_runs r
    ...
    SELECT title,
           failed_count,
           RANK() OVER (ORDER BY failed_count DESC) AS rank_position
    FROM failed_by_case
...
SELECT id,
           engineer_id,
           executed_at,
           ROW_NUMBER() OVER (PARTITION BY engineer_id ORDER BY executed_at) AS seq_no
    FROM test_runs
    ORDER BY engineer_id, seq_no;
