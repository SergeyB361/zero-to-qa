...
def rank_cases_by_failures(conn: sqlite3.Connection):
    """Верни пары `title:rank` по числу failed runs."""
    # Напиши SQL или Python + SQL по условию.
    return []


def row_number_runs_per_engineer(conn: sqlite3.Connection):
    """Верни пары `run_id:row_number` внутри engineer_id по времени."""
    # Напиши SQL или Python + SQL по условию.
    return []


def dense_rank_endpoints_by_latency(conn: sqlite3.Connection):
    """Верни пары `endpoint:rank` по средней latency."""
    # Напиши SQL или Python + SQL по условию.
    return []
...
def main() -> None:
    conn = build_demo_db()
    print('rank_cases_by_failures ->', rank_cases_by_failures(conn), '| expected:', "['Create order:1', 'Login works:2', 'Refund order:2']")
    print('row_number_runs_per_engineer ->', row_number_runs_per_engineer(conn), '| expected:', "['1:1', '2:1', '3:2', '4:2', '5:3', '6:1', '7:4', '8:1', '9:3', '10:2', '11:5']")
    print('dense_rank_endpoints_by_latency ->', dense_rank_endpoints_by_latency(conn), '| expected:', "['/payments/refund:1', '/reports:2', '/orders:3', '/login:4']")
