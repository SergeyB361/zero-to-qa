...
def status_counts_per_area(conn: sqlite3.Connection):
    """Верни пары `area:passed:failed`."""
    # Напиши SQL или Python + SQL по условию.
    return []


def severity_open_closed(conn: sqlite3.Connection):
    """Верни пары `severity:open:closed`."""
    # Напиши SQL или Python + SQL по условию.
    return []


def api_status_buckets(conn: sqlite3.Connection):
    """Верни словарь bucket -> count для api_checks."""
    # Напиши SQL или Python + SQL по условию.
    return {}
...
def main() -> None:
    conn = build_demo_db()
    print('status_counts_per_area ->', status_counts_per_area(conn), '| expected:', "['admin:1:0', 'auth:2:1', 'catalog:1:0', 'checkout:1:2', 'payments:1:1']")
    print('severity_open_closed ->', severity_open_closed(conn), '| expected:', "['critical:1:0', 'major:1:1', 'minor:1:0']")
    print('api_status_buckets ->', api_status_buckets(conn), '| expected:', "{'2xx': 4, '4xx': 1, '5xx': 2}")
