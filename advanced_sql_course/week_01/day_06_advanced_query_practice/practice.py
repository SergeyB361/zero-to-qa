...
def top_slowest_cases(conn: sqlite3.Connection):
    """Верни top-3 test_cases по средней длительности."""
    # Напиши SQL или Python + SQL по условию.
    return []


def releases_with_failed_runs(conn: sqlite3.Connection):
    """Верни пары `build_tag:failed_count` только для релизов с ошибками."""
    # Напиши SQL или Python + SQL по условию.
    return []


def engineer_failure_share(conn: sqlite3.Connection):
    """Верни пары `engineer:failed_runs`."""
    # Напиши SQL или Python + SQL по условию.
    return []
...
def main() -> None:
    conn = build_demo_db()
    print('top_slowest_cases ->', top_slowest_cases(conn), '| expected:', "['Export report', 'Refund order', 'Create order']")
    print('releases_with_failed_runs ->', releases_with_failed_runs(conn), '| expected:', "['build-101:1', 'build-102:2', 'build-104:1']")
    print('engineer_failure_share ->', engineer_failure_share(conn), '| expected:', "['Anna:1', 'Boris:2', 'Nina:1', 'Oleg:0']")
