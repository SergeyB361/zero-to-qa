...
-- Пример 2
WITH avg_case_duration AS (
        SELECT c.title,
               AVG(r.duration_sec) AS avg_duration
        FROM test_runs r
    ...
    SELECT title,
           avg_duration,
           RANK() OVER (ORDER BY avg_duration DESC) AS duration_rank
    FROM avg_case_duration
    ORDER BY duration_rank, title;

-- Пример 3
WITH failed_by_release AS (
        SELECT release_id, COUNT(*) AS failed_count
        FROM test_runs
        WHERE status = 'failed'
        GROUP BY release_id
    )
    SELECT rel.build_tag, f.failed_count
    FROM failed_by_release f
    JOIN releases rel ON rel.id = f.release_id
    ORDER BY rel.deployed_at;
