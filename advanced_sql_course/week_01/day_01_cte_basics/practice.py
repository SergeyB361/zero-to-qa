...
def failed_runs_per_case(conn: sqlite3.Connection):
    """Верни пары `title:failed_count` через CTE."""
    # Напиши SQL или Python + SQL по условию.
    return []


def latest_release_runs(conn: sqlite3.Connection):
    """Верни количество test_runs для последнего релиза."""
    # Напиши SQL или Python + SQL по условию.
    return -1


def open_defects_per_release(conn: sqlite3.Connection):
    """Верни пары `build_tag:open_defects`."""
    # Напиши SQL или Python + SQL по условию.
    return []
...
def main() -> None:
    conn = build_demo_db()
    print('failed_runs_per_case ->', failed_runs_per_case(conn), '| expected:', "['Create order:2', 'Login works:1']")
    print('latest_release_runs ->', latest_release_runs(conn), '| expected:', '3')
    print('open_defects_per_release ->', open_defects_per_release(conn), '| expected:', "['build-102:1', 'build-103:1', 'build-104:1']")
