...
SELECT build_tag,
           deployed_at,
           LAG(deployed_at) OVER (ORDER BY deployed_at) AS previous_release_date,
           LEAD(deployed_at) OVER (ORDER BY deployed_at) AS next_release_date
    FROM releases
...
SELECT id,
           ...
           SUM(CASE WHEN status = 'failed' THEN 1 ELSE 0 END)
               OVER (ORDER BY executed_at ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW) AS cumulative_failed
    FROM test_runs
