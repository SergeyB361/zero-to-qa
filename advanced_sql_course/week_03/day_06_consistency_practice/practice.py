...
def resolve_defect_in_transaction(conn: sqlite3.Connection):
    """Закрой defect id = 1 и добавь audit event в одной транзакции."""
    # Напиши SQL или Python + SQL по условию.
    return ''


def rollback_on_invalid_owner(conn: sqlite3.Connection):
    """Верни True, если invalid insert не меняет количество test_cases."""
    # Напиши SQL или Python + SQL по условию.
    return False


def audit_count_after_resolution(conn: sqlite3.Connection):
    """Верни количество audit events для defect после успешного закрытия."""
    # Напиши SQL или Python + SQL по условию.
    return -1
...
def main() -> None:
    conn = build_demo_db()
    print('resolve_defect_in_transaction ->', resolve_defect_in_transaction(conn), '| expected:', 'closed')
    print('rollback_on_invalid_owner ->', rollback_on_invalid_owner(conn), '| expected:', 'True')
    print('audit_count_after_resolution ->', audit_count_after_resolution(conn), '| expected:', '2')
