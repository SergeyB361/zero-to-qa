...
def main() -> None:
    conn = build_demo_db()
    print(fetch_all(conn, "SELECT COUNT(*) AS cnt FROM audit_log WHERE entity_type = 'defect' AND entity_id = 1"))
    conn.execute("BEGIN")
    conn.execute("UPDATE defects SET status = 'closed', resolved_at = '2026-04-05' WHERE id = 1")
    conn.execute("INSERT INTO audit_log (id, entity_type, entity_id, action, actor_id, created_at) VALUES (10, 'defect', 1, 'closed', 2, '2026-04-05 10:00:00')")
    conn.commit()
    print(fetch_all(conn, "SELECT status, resolved_at FROM defects WHERE id = 1"))
    print(fetch_all(conn, "SELECT COUNT(*) AS cnt FROM audit_log WHERE entity_type = 'defect' AND entity_id = 1"))
