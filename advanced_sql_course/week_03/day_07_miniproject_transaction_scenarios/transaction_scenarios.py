...
def resolve_defect(conn: sqlite3.Connection):
    """Закрывает defect и пишет audit event."""
    return ''


def try_invalid_case_insert(conn: sqlite3.Connection):
    """Проверяет rollback при invalid owner."""
    return False


def defect_audit_count(conn: sqlite3.Connection):
    """Считает audit events по defect."""
    return -1


def main() -> None:
    conn = build_demo_db()
    print('resolve_defect ->', resolve_defect(conn), '| expected:', 'closed')
    print('try_invalid_case_insert ->', try_invalid_case_insert(conn), '| expected:', 'True')
    print('defect_audit_count ->', defect_audit_count(conn), '| expected:', '2')
