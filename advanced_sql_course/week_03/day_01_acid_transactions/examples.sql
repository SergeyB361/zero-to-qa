...
-- Пример 2
BEGIN; UPDATE defects SET status = 'closed' WHERE id = 1; COMMIT;

-- Пример 3
SELECT id, status FROM defects WHERE id = 1;

-- Пример 4
BEGIN; UPDATE defects SET status = 'closed' WHERE id = 2; ROLLBACK;

-- Пример 5
SELECT id, status FROM defects WHERE id = 2;
