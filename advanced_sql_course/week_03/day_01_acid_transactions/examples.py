...
def main() -> None:
    conn = build_demo_db()
    conn.executescript("BEGIN; UPDATE defects SET status = 'closed' WHERE id = 1; COMMIT;")
    print(fetch_all(conn, "SELECT id, status FROM defects WHERE id = 1"))
    conn.executescript("BEGIN; UPDATE defects SET status = 'closed' WHERE id = 2; ROLLBACK;")
    print(fetch_all(conn, "SELECT id, status FROM defects WHERE id = 2"))
