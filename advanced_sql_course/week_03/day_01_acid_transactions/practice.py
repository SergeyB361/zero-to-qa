...
def commit_defect_resolution(conn: sqlite3.Connection):
    """Закрой defect id = 1 в транзакции и верни итоговый статус."""
    # Напиши SQL или Python + SQL по условию.
    return ''


def rollback_defect_resolution(conn: sqlite3.Connection):
    """Попробуй закрыть defect id = 2 и откати. Верни итоговый статус."""
    # Напиши SQL или Python + SQL по условию.
    return ''


def count_open_defects_after_demo(conn: sqlite3.Connection):
    """Верни количество open defects после rollback-сценария."""
    # Напиши SQL или Python + SQL по условию.
    return -1
...
def main() -> None:
    conn = build_demo_db()
    print('commit_defect_resolution ->', commit_defect_resolution(conn), '| expected:', 'closed')
    print('rollback_defect_resolution ->', rollback_defect_resolution(conn), '| expected:', 'open')
    print('count_open_defects_after_demo ->', count_open_defects_after_demo(conn), '| expected:', '3')
