...
def main() -> None:
    ...
    try:
        conn2.execute("UPDATE defects SET status = 'closed' WHERE id = 2")
        conn2.commit()
        print('second writer succeeded unexpectedly')
    except sqlite3.OperationalError as exc:
        print(str(exc))
    conn1.commit()
    conn2.execute("UPDATE defects SET status = 'closed' WHERE id = 2")
    conn2.commit()
    print(fetch_all(conn2, "SELECT id, status FROM defects WHERE id = 2"))
