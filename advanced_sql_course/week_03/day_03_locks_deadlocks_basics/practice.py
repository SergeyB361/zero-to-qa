...
def lock_error_happens(conn: sqlite3.Connection):
    """Верни True, если второй writer ловит `database is locked`."""
    # Напиши SQL или Python + SQL по условию.
    return False


def sqlite_deadlock_note(conn: sqlite3.Connection):
    """Верни строку о том, что deadlocks чаще обсуждаются в server DB."""
    # Напиши SQL или Python + SQL по условию.
    return ''


def released_lock_allows_write(conn: sqlite3.Connection):
    """Верни True, если после commit второй writer может обновить строку."""
    # Напиши SQL или Python + SQL по условию.
    return False
...
def main() -> None:
    conn = build_demo_db()
    print('lock_error_happens ->', lock_error_happens(conn), '| expected:', 'True')
    print('sqlite_deadlock_note ->', sqlite_deadlock_note(conn), '| expected:', 'Deadlocks are usually discussed in server databases')
    print('released_lock_allows_write ->', released_lock_allows_write(conn), '| expected:', 'True')
