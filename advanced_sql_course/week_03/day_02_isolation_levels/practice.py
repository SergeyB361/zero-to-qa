...
def reader_sees_old_value(conn: sqlite3.Connection):
    """Верни True, если второй connection не видит незакоммиченный status."""
    # Напиши SQL или Python + SQL по условию.
    return False


def value_after_commit(conn: sqlite3.Connection):
    """Верни статус после commit в первом connection."""
    # Напиши SQL или Python + SQL по условию.
    return ''


def sqlite_isolation_note(conn: sqlite3.Connection):
    """Верни короткую строку-заметку про SQLite isolation."""
    # Напиши SQL или Python + SQL по условию.
    return ''
...
def main() -> None:
    conn = build_demo_db()
    print('reader_sees_old_value ->', reader_sees_old_value(conn), '| expected:', 'True')
    print('value_after_commit ->', value_after_commit(conn), '| expected:', 'closed')
    print('sqlite_isolation_note ->', sqlite_isolation_note(conn), '| expected:', 'SQLite uses its own locking model')
