...
def main() -> None:
    conn = build_demo_db()
    try:
        conn.execute("INSERT INTO test_cases (id, title, area, priority, owner_id) VALUES (100, 'Broken owner', 'auth', 'low', 999)")
        conn.commit()
    except sqlite3.IntegrityError as exc:
        print('fk_error ->', str(exc))
    conn.execute("CREATE TABLE temp_users (id INTEGER PRIMARY KEY, email TEXT UNIQUE)")
    conn.execute("INSERT INTO temp_users VALUES (1, 'a@example.com')")
    try:
        conn.execute("INSERT INTO temp_users VALUES (2, 'a@example.com')")
        conn.commit()
    except sqlite3.IntegrityError as exc:
        print('unique_error ->', str(exc))
