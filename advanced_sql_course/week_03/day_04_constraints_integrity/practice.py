...
def foreign_key_violation_detected(conn: sqlite3.Connection):
    """Верни True, если вставка несуществующего owner_id приводит к ошибке."""
    # Напиши SQL или Python + SQL по условию.
    return False


def unique_violation_detected(conn: sqlite3.Connection):
    """Создай временную таблицу с UNIQUE и поймай ошибку дубликата."""
    # Напиши SQL или Python + SQL по условию.
    return False


def check_constraint_violation(conn: sqlite3.Connection):
    """Создай временную таблицу с CHECK и поймай ошибку."""
    # Напиши SQL или Python + SQL по условию.
    return False
...
def main() -> None:
    conn = build_demo_db()
    print('foreign_key_violation_detected ->', foreign_key_violation_detected(conn), '| expected:', 'True')
    print('unique_violation_detected ->', unique_violation_detected(conn), '| expected:', 'True')
    print('check_constraint_violation ->', check_constraint_violation(conn), '| expected:', 'True')
