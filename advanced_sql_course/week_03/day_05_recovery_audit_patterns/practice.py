...
def audit_events_for_defects(conn: sqlite3.Connection):
    """Верни пары `entity_id:action` для defect events."""
    # Напиши SQL или Python + SQL по условию.
    return []


def open_defects_with_owner(conn: sqlite3.Connection):
    """Верни пары `defect:owner` для open defects."""
    # Напиши SQL или Python + SQL по условию.
    return []


def soft_close_summary(conn: sqlite3.Connection):
    """Верни количество closed defects."""
    # Напиши SQL или Python + SQL по условию.
    return -1
...
def main() -> None:
    conn = build_demo_db()
    print('audit_events_for_defects ->', audit_events_for_defects(conn), '| expected:', "['1:created', '3:closed']")
    print('open_defects_with_owner ->', open_defects_with_owner(conn), '| expected:', "['Login returns 401:Boris', 'Refund 502:Nina', 'Slow export:Oleg']")
    print('soft_close_summary ->', soft_close_summary(conn), '| expected:', '1')
