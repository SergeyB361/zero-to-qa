...
def main() -> None:
    conn = build_demo_db()
    print(fetch_all(conn, "SELECT entity_id, action, created_at FROM audit_log WHERE entity_type = 'defect' ORDER BY created_at"))
    print(fetch_all(conn, "SELECT title, status, resolved_at FROM defects ORDER BY id"))
